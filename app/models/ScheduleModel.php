...
class ScheduleModel extends BaseModel {
    ...
    if ($result && $result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            // ✅ Update req_status to "On Going" if travel_date is today
            if ($row['travel_date'] === $today && $row['req_status'] === 'Approved') {
                $update = $this->db->prepare("
                    UPDATE vehicle_request_assignment 
                    SET req_status = 'On Going' 
                    WHERE control_no = ?
                ");
                if ($update) {
                    $update->bind_param("i", $row['control_no']);
                    $update->execute();
                    $update->close();
                    $row['req_status'] = 'On Going'; // reflect immediately in this array
                }
            }
