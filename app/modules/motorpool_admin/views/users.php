        ...
        <div x-show="showDetails" x-cloak class="bg-white shadow rounded-lg p-4 max-h-[640px] overflow-y-auto">
          ...
          <form id="userForm" method="post" action="../../../controllers/UserAdminController.php" class="space-y-4">
            ...
            <div class="flex justify-center gap-2 pt-2">
              <button type="button" class="btn btn-secondary" @click="viewRequestHistory(selected.requester_id)">
  ...
  <script>
    async function viewRequestHistory(requester_id) {
      ...
      try {
        ...
        const res = await fetch("../../../controllers/UserAdminController.php", { method: "POST", body: formData });
        const history = await res.json();

        if (!Array.isArray(history) || !history.length) {
          Swal.fire({ icon: "info", title: "No Work History Found", text: "This requester has no recorded requests yet." });
          return;
        }
