...
?> -->
    ...
    <div class="flex justify-center gap-4">
      <form method="GET" class="items-center" id="filterForm">
        ...
        <ul class="flex w-[231px] mx-auto items-center text-sm bg-red-100 rounded-2xl mb-3 text-primary">
          <li>
          ...
          <article class="w-4/5 md:w-1/2 m-5 mx-auto rounded-lg border border-gray-200 bg-white p-4 shadow-sm hover:shadow-lg transition sm:p-6">
            <?php if ($type === 'repair'): ?>
              ...
              <h3 class="text-base font-semibold text-gray-800">
                ...
                <span class="ml-1 mt-2 text-xs">
                  <?php
                    $req_status = htmlspecialchars($track['req_status']);
                    $statusClass = match($req_status) {
                      "To Inspect" => "bg-yellow-100 text-yellow-700 font-medium text-xs px-3 w-20",
                      "In Progress" => "bg-blue-100 text-blue-700 font-medium text-xs px-3 w-20",
                      "Completed"  => "bg-green-100 text-green-700 font-medium text-xs px-3",
                      default => "bg-gray-100 text-gray-700 font-medium text-xs px-3",
                    };
                  ?>
              ...
              <!-- Hidden Form to carry tracking_id --> 
                <form action="feedback.php" method="GET" class="hidden" id="form_<?php echo $track['tracking_id']; ?>"> 
                  ...
                  <?php if ($status === 'Pending'): ?>
                   <button class="btn btn-danger mt-2" onclick="openCancelModal(<?= $track['control_no']; ?>)">Cancel Request</button>
